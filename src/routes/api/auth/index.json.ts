...
export const put = async ({ url, request, locals }) => {
	console.log('🚀 ~ file: index.json.ts ~ line 109 ~ put ~ url', url);
	try {
		if (!locals?.user?._id) {
			return {
				status: 401,
		...
		console.log('🚀 ~ file: index.json.ts ~ line 121 ~ put ~ queryParams', queryParams);

		const userUpdate = await request.json();
		console.log('🚀 ~ file: index.json.ts ~ line 121 ~ put ~ userUpdate', userUpdate);
		const userUpdated = await ContactsModel.findByIdAndUpdate({ _id: queryParams.id }, userUpdate, {
			new: true
