<script lang="ts">
	...
	const getUpdateUser = async (paramsObj, finalData) => {
		try {
			let searchParams = new URLSearchParams(paramsObj as string);
			const res = await fetch('/api/auth.json?' + searchParams.toString(), {
				method: 'PUT',
	...
	const getDeleteUser = async (paramsObj) => {
		try {
			let searchParams = new URLSearchParams(paramsObj as string);
			const res = await fetch('/api/auth.json?' + searchParams.toString(), {
				method: 'PUT',
	...
	const heandleEditable = async (id: any, editable: boolean) => {
		contacts.forEach(async (list) => {
			if (list._id === id && editable === true) {
				await getUpdateUser({ id: id }, list);
			}
	...
	const heandleDelete = async (id: any) => {
